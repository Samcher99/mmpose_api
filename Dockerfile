...
# 定義容器啟動時執行的命令。
# uvicorn app:app 表示運行 app.py 文件中名為 'app' 的 FastAPI 應用。
# --host 0.0.0.0 讓服務器監聽所有可用的網絡接口，允許外部訪問。
# --port 8000 設定服務器監聽的端口為 8000。
# 用docker compose 決定該做啥
#CMD ["uvicorn", "app:app", "--host", "0.0.0.0", "--port", "8000"]

#我這個image建起來了 現在我想用docker compose 把這個image變成容器並持續執行 並開啟8000 port對外連線 注意這個容器要有gpu服務
#我希望在我當前資料夾新增修改app.py 能後被docker環境存取到